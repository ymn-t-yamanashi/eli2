...
    var modelOrVrm = this.v[name];
    ...
    // --- 【修正】手動回転を保持するリストを初期化・更新 ---
    if (!this.v._manualBoneRotations) this.v._manualBoneRotations = {};
    const key = `${name}::${boneName}`; // モデル名とボーン名で一意なキーを作成
    
    // 回転値を保持（renderループで再適用するために必要）
    this.v._manualBoneRotations[key] = { x, y, z };
    // --------------------------------------------------------
...
      console.warn("bone '" + boneName + "' not found on model '" + name + "'");
      delete this.v._manualBoneRotations[key]; // 見つからないなら保持データも削除
      return;
    }
    // 回転を即座に適用（初回適用）
...
    // VRM の場合は .scene から辿る
    var root = (model.scene && model.scene instanceof THREE.Object3D) ? model.scene : model;
    root.traverse((obj) => { if (obj.isBone) t.pushEvent('get_bone', { name: obj.name }) });
  },
...
    context.font = `Bold ${fontSize}px ${fontFamily}`; // サイズ測定前にフォントを適用
...
      textMesh.geometry.needsUpdate = true; // Three.jsにジオメトリ更新を通知
    }
...
    }
    ...
    // --- 【修正】手動回転の保持データとVRMリストからも削除 ---
    if (this.v._manualBoneRotations) {
        // 該当モデル名のボーン回転データを全て削除
        for (const key in this.v._manualBoneRotations) {
            if (key.startsWith(`${name}::`)) {
                delete this.v._manualBoneRotations[key];
            }
        }
    }
    // VRMリストからも削除（VRMの場合のみ）
    if (this.v._vrms && objectToRemove.userData && objectToRemove.userData.vrm) {
        this.v._vrms = this.v._vrms.filter(vrm => vrm !== objectToRemove);
    }
    // --------------------------------------------------------

    // シーンからオブジェクトを削除
    // VRMの場合は .scene を削除
    const sceneObject = (objectToRemove.scene && objectToRemove.scene instanceof THREE.Object3D) ? objectToRemove.scene : objectToRemove;
    if (sceneObject) {
        this.v.scene.remove(sceneObject);
    }
...
    // this.v からも参照を削除
    delete this.v[name];
    delete this.v[name + '_gltf']; // VRMロード時に保存している場合
...
    el.appendChild(v["renderer"].domElement);
    ...
    // --- 【修正】Clockをvに保存 ---
    v["clock"] = new THREE.Clock(); // 時間管理用のClockをvに保存

    function render() {
      requestAnimationFrame(render);
        
        const deltaTime = v["clock"].getDelta(); // 前フレームからの経過時間を取得

        // 1. VRMのSecondary Animation（揺れもの）の更新を実行
        if (v._vrms) {
            v._vrms.forEach(vrm => {
        ...
        // 2. 手動で指定されたボーンの回転を再適用 (VRMアップデートを上書き)
        if (v._manualBoneRotations) {
            for (const key in v._manualBoneRotations) {
                const parts = key.split('::');
                const modelName = parts[0];
                const boneName = parts[1];
                const rot = v._manualBoneRotations[key];

                const modelOrVrm = v[modelName];
                if (!modelOrVrm) continue;
                ...
                if (bone) {
                    ...
                    if (rot.y != null) bone.rotation.y = rot.y;
